...
| ~~M7~~ | `LeWM port — GitHub Actions Python CI + pre-commit + ruff` | **landed (PR #8).** `.github/workflows/python.yml` runs three jobs on every push to `main` / PR touching the Python tree: (1) `lint` runs `ruff check tools/` + `ruff format --check tools/` against the pinned `ruff==0.6.9`; (2) `smoke` runs `py_compile` on every `.py` under `tools/` and the eight verification commands from `tools/lewm/README.md` (`tools.lewm.train --smoke`, the five `tools/lewm/tests/test_*.py` smokes, `tools/world_model/train_world_model.py --smoke --epochs 2`); (3) `benchmark-smoke` re-trains a fresh checkpoint and drives `tools/lewm/benchmark.py` over all five modes for one episode each, then uploads the resulting `benchmark.csv` + `benchmark.summary.csv` as a CI artifact. `pyproject.toml` ships a shared ruff config (`line-length = 100`, `target-version = "py310"`, the standard pyflakes / pycodestyle / isort / bugbear / pyupgrade / pylint-subset / ruff-specific rule set, vendored `tools/lewm/module.py` exempt from both lint and format to keep it byte-identical to upstream `c8a4417`, `tools/world_model/` excluded since it predates the lint config). `.pre-commit-config.yaml` wires up `trailing-whitespace`, `end-of-file-fixer`, `check-yaml`, `check-merge-conflict`, `check-added-large-files` (max 2 MB), `ruff` and `ruff-format`. The hygiene hooks are explicitly scoped to `tools/lewm/*.py`, `tools/lewm/requirements*.txt`, `pyproject.toml`, `.pre-commit-config.yaml` and `.github/workflows/*.yml` so they cannot rewrite Unity-managed assets. `tools/lewm/requirements-dev.txt` pins `ruff==0.6.9` and `pre-commit>=3.7`. |
| ~~M8b~~ | `LeWM port — FPT AI Factory one-shot training script` | **landed (this PR).** `tools/lewm/scripts/train_fpt_h100.sh` wraps the full FPT recipe from the architecture plan (`docs/research/lewm-fpt-aifactory-plan.md`, also vendored in this PR) into a single command: installs `tools/lewm/requirements.txt`, asserts `torch.cuda.is_available()` (refuses to fall back to CPU when `DEVICE=cuda` so a misinstalled wheel cannot silently burn GPU budget), runs the M1 synthetic smoke against the GPU, runs the full M4 training command with cosine LR + 10 % val split + per-epoch metrics CSV + `best.pt` tracking, then runs the M6 5-mode benchmark and prints the summary CSV inline. All knobs (`EPOCHS`, `BATCH_SIZE`, `VAL_SPLIT`, `WARMUP_STEPS`, `MIN_LR_RATIO`, `BENCHMARK_EPISODES`, `BENCHMARK_MAX_STEPS`, `SEED`, `DEVICE`, `OUTPUT_DIR`, `SKIP_SMOKE`, `SKIP_BENCHMARK`, `ARCHIVE`) are env vars with defaults that match the recommended recipe. `--synthetic` as the positional arg exercises only the smoke step (useful while still waiting on Unity gameplay JSONL). `ARCHIVE=1` tars `OUTPUT_DIR` into `lewm-artifacts-<timestamp>.tar.gz` for one-click Jupyter download. Cost reference at the AI Notebook 1× H100 rate ($2.31/hour, billed per-second): happy-path full pipeline ≈ $2 USD / ~20 min wall clock. No new Python deps; no Unity changes; no CI changes (the script is intentionally not exercised by the GitHub Actions `lint` / `smoke` / `benchmark-smoke` jobs since it requires CUDA). |
